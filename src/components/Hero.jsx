...
import { StarsBackground } from "./animate-ui/components/backgrounds/stars"; // Adjust path if needed


const HeroSection = () => {
  // Simple check for dark mode in standard React
  ...
  return (
    ...
    >
      {/* BACKGROUND LAYER - Placed first with absolute positioning */}
      <div className="absolute inset-0 z-0">
        <StarsBackground
          starColor={isDarkMode ? "#fbbf24" : "#ffffff"}
          // className={cn(
          //   'h-full w-full',
          //   'bg-[radial-gradient(ellipse_at_bottom,_#1e293b_0%,_#020617_100%)]'
          // )}
        />
      </div>

      {/* CONTENT LAYER - Must be relative and z-10 to stay on top */}
      <div className="relative z-10 flex flex-col md:flex-row items-start md:items-center gap-8 md:gap-12 lg:gap-16 w-full max-w-7xl">
        ...
        <div className="flex-1 md:flex-[1.2] text-center md:text-left space-y-5 md:space-y-6 w-full">
          {/* SYSTEM BADGE */}
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-md bg-white/5 border border-white/10 text-[10px] sm:text-[11px] tracking-widest font-medium text-amber-300 backdrop-blur-md">
          ...
          <div className="space-y-3 max-w-xl mx-auto md:mx-0">
            ...
            <div className="flex justify-center md:justify-start flex-wrap gap-2">
              {modules.map((tech) => (
                <span
                  key={tech}
                  className="px-2.5 sm:px-3 py-1 rounded-md bg-white/5 border border-white/10 text-xs sm:text-sm text-slate-300 whitespace-nowrap backdrop-blur-sm"
                >
                  {tech}
                </span>
              ))}
          ...
          <Button
            ...
            className="mt-2 cursor-pointer h-auto px-8 sm:px-10 md:px-12 py-2.5 sm:py-3 rounded-xl bg-amber-500/10 text-amber-300 font-semibold border border-amber-500/30 hover:bg-amber-500/20 hover:border-amber-400/50 transition text-sm sm:text-base active:scale-95"
          >
            Explore My Work
          ...
          <div className="mt-6 text-center md:text-left">
            <span className="text-[10px] sm:text-[11px] font-mono text-slate-400 animate-pulse">
              [user@archlinux ~]$ echo " I use Arch ! "
            </span>
          </div>
        </div>
